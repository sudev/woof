...
class SynchronousFeedConsumer:
    """Threaded gomsg feed consumer
    callbacks are called on a same thread in the same method
    ...
    def add_topic(self, topic, todo, processing_data, parts=None):
        """
        Set the topic/partitions to consume
        ...
        """
        self.callbacks[topic] = (todo, processing_data)
    ...
    def get_topic_info(self):
        while True:
            try:
                for m in self.cons.fetch_messages():
                    break_val = self.callbacks[m.topic][0](m.key, m.value, self.callbacks[m.topic][1])
                    if break_val:
                        return
                    self.cons.task_done(m)
            except:
